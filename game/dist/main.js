...
class Game {
    ...
    async showMainMenu() {
        ...
        if (choice === 1) {
            ...
            const saveInfo = save_1.SaveManager.getSaveInfo(1);
            if (saveInfo) {
                this.player = save_1.SaveManager.load(1);
                if (this.player) {
    ...
    async newGame() {
        ...
        const chain = pokemon_1.PokemonManager.getEvolutionChain(starter.baseId);
        console.log('');
        console.log(`欢迎 ${name}！`);
        console.log(`你获得了 ${pokemon_1.PokemonManager.getPokemonName(starter)}！`);
    ...
    async showLocationMenu(saveSlot) {
        ...
        const expInfo = pokemon_1.PokemonManager.getExpToNextLevel(lead);
        const pcCount = player_1.PlayerManager.getPcCount(this.player);
        console.log(`队伍: ${pokemon_1.PokemonManager.getPokemonName(lead)} Lv.${lead.level} | PC: ${pcCount}只`);
    ...
    async managePC() {
        ...
        while (true) {
            console.log('');
            const pcLines = player_1.PlayerManager.viewPC(this.player);
            ...
            if (choice === 1) {
                ...
                for (let j = 0; j < box.length; j++) {
                    const p = box[j];
                    console.log(`  [${j + 1}] ${pokemon_1.PokemonManager.getPokemonName(p)} Lv.${p.level}`);
                }
                console.log('  [0] 取消');
                const pokeChoice = await this.input.getMenuChoice(box.length);
                if (pokeChoice === 0)
                    continue;
                const result = player_1.PlayerManager.withdrawFromPC(this.player, boxChoice - 1, pokeChoice - 1);
            ...
            if (choice === 2) {
                ...
                for (let i = 0; i < this.player.party.length; i++) {
                    ...
                    console.log(`  [${i + 1}] ${pokemon_1.PokemonManager.getPokemonName(p)} Lv.${p.level}`);
                }
                console.log('  [0] 取消');
                const partyChoice = await this.input.getMenuChoice(this.player.party.length);
                if (partyChoice === 0)
                    continue;
                const result = player_1.PlayerManager.depositToPC(this.player, partyChoice - 1);
                console.log(result.message);
                await this.input.waitForKey();
            }
            if (choice === 3) {
                ...
                for (let i = 0; i < allPokemon.length; i++) {
                    ...
                    const name = pokemon_1.PokemonManager.getPokemonName(entry.pokemon);
                ...
                const targetName = pokemon_1.PokemonManager.getPokemonName(target.pokemon);
                console.log(`\n确定要释放 ${targetName} 吗？`);
    ...
    async explore(location, saveSlot) {
        ...
        switch (event.type) {
            ...
            case 'item':
                ...
                player_1.PlayerManager.addItem(this.player, event.reward.id, event.reward.qty);
                await this.input.waitForKey();
                break;
            case 'npc':
                console.log(event.message);
            ...
            case 'money':
                console.log(event.message);
                player_1.PlayerManager.addMoney(this.player, event.reward);
                await this.input.waitForKey();
                break;
            case 'heal':
                console.log(event.message);
                player_1.PlayerManager.healAllPokemon(this.player);
                await this.input.waitForKey();
                break;
            default:
                console.log(event.message);
    ...
    async handleWildEncounter(location, saveSlot) {
        if (!this.player)
            return;
        const minTier = location.minTier || 1;
        const maxTier = location.maxTier || 2;
        const rareCheck = pokemon_1.PokemonManager.getRareEncounter(minTier * 2);
        let enemyPokemon = rareCheck || pokemon_1.PokemonManager.getRandomWildPokemon(minTier, maxTier);
        if (!enemyPokemon) {
            console.log('什么也没发现...');
            await this.input.waitForKey();
            return;
    ...
    async handleTrainerBattle(location, saveSlot) {
        if (!this.player)
            return;
        const minTier = location.minTier || 1;
        const maxTier = location.maxTier || 2;
        const enemyPokemon = pokemon_1.PokemonManager.getRandomWildPokemon(minTier, maxTier);
        if (!enemyPokemon)
            return;
    ...
    async handleRareEncounter(location, saveSlot) {
        if (!this.player)
            return;
        const minTier = location.minTier || 1;
        const rarePokemon = pokemon_1.PokemonManager.getRareEncounter(minTier * 2);
        if (!rarePokemon) {
            console.log('神秘气息消失了...');
            await this.input.waitForKey();
            return;
    ...
    async wildBattle(enemyPokemon, saveSlot, isRare = false) {
        if (!this.player)
            return;
        ...
        await this.input.waitForKey();
        const state = battle_1.BattleEngine.createBattle(playerPokemon, enemyPokemon, 'wild', this.player.party);
        while (!state.isOver) {
            renderer_1.Renderer.drawBattle(state);
            renderer_1.Renderer.drawBattleMenu();
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    if (ballResult) {
                        renderer_1.Renderer.drawCatchAttempt(ballResult.shakes, ballResult.success);
                        await this.input.waitForKey();
                        if (ballResult.success) {
                            const result = player_1.PlayerManager.addPokemon(this.player, state.enemyPokemon);
                            console.log(`${pokemon_1.PokemonManager.getPokemonName(state.enemyPokemon)} 被捕捉了！`);
                            console.log(result.message);
                            if (isRare) {
                    ...
                    break;
                case 3:
                    const aliveCount = this.player.party.filter(p => !pokemon_1.PokemonManager.isFainted(p)).length;
                    if (aliveCount <= 1) {
                    ...
                    for (let i = 0; i < this.player.party.length; i++) {
                        const p = this.player.party[i];
                        const fainted = pokemon_1.PokemonManager.isFainted(p) ? '(濒死)' : '';
                        const current = p === state.playerPokemon ? ' ← 当前' : '';
                        console.log(`  [${i + 1}] ${pokemon_1.PokemonManager.getPokemonName(p)} Lv.${p.level} HP:${p.currentHp}/${p.stats.hp}${fainted}${current}`);
                    }
                    console.log('  [0] 取消');
                    ...
                    if (pokemon_1.PokemonManager.isFainted(switchTarget)) {
            ...
            const result = battle_1.BattleEngine.executeTurn(state, playerAction, enemyAction);
            for (const msg of result.messages) {
                console.log(msg);
            }
            if (state.playerPokemon.currentHp <= 0 && battle_1.BattleEngine.hasAlivePartyMember(state)) {
                console.log(`${pokemon_1.PokemonManager.getPokemonName(state.playerPokemon)} 倒下了...`);
                console.log('');
                console.log('选择下一只宝可梦:');
                const aliveParty = this.player.party.filter(p => !pokemon_1.PokemonManager.isFainted(p));
                for (let i = 0; i < aliveParty.length; i++) {
                    const p = aliveParty[i];
                    console.log(`  [${i + 1}] ${pokemon_1.PokemonManager.getPokemonName(p)} Lv.${p.level} HP:${p.currentHp}/${p.stats.hp}`);
                }
                console.log('  [0] 逃跑');
                console.log('');
            ...
            if (result.isOver && state.winner === 'player') {
                console.log(`${pokemon_1.PokemonManager.getPokemonName(state.enemyPokemon)} 被击败了！`);
            }
        ...
        if (state.winner === 'player') {
            ...
            for (const p of aliveParty) {
                const gainResult = pokemon_1.PokemonManager.gainExp(p, Math.floor(exp / aliveParty.length));
                for (const msg of gainResult.messages) {
                    console.log(msg);
                }
            }
            const moneyReward = Math.floor(Math.random() * 100) + 20;
            player_1.PlayerManager.addMoney(this.player, moneyReward);
    ...
    async trainerBattle(enemyPokemon, saveSlot) {
        if (!this.player)
            return;
        const playerPokemon = player_1.PlayerManager.getFirstHealthyPokemon(this.player);
        if (!playerPokemon)
            return;
        const state = battle_1.BattleEngine.createBattle(playerPokemon, enemyPokemon, 'trainer', this.player.party);
        while (!state.isOver) {
            renderer_1.Renderer.drawBattle(state);
            renderer_1.Renderer.drawBattleMenu();
            const choice = await this.input.getMenuChoice(4);
            ...
            if (result.isOver && state.winner === 'player') {
                console.log(`击败了训练家的 ${pokemon_1.PokemonManager.getPokemonName(state.enemyPokemon)}！`);
            }
            await this.input.waitForKey();
        }
        if (state.winner === 'player') {
            const aliveParty = this.player.party.filter(p => !pokemon_1.PokemonManager.isFainted(p));
            const exp = Math.floor(battle_1.BattleEngine.getExpReward(state) * 1.5);
    ...
    async useItem(itemId, state) {
        ...
        return battle_1.BattleEngine.canCatch(state, 1.0);
    }
    syncPlayerPokemon(state) {
        if (!this.player)
            return;
        const index = this.player.party.findIndex(p => p.baseId === state.playerPokemon.baseId);
        if (index >= 0) {
            this.player.party[index] = state.playerPokemon;
        }
    }
    async move(location, saveSlot) {
        if (!this.player)
            return;
    ...
    async pokemonCenter() {
        ...
        console.log('');
        const needsHealing = this.player.party.some(p => p.currentHp < p.stats.hp || p.statusEffects.length > 0);
        if (!needsHealing) {
        ...
        for (let i = 0; i < this.player.party.length; i++) {
            const p = this.player.party[i];
            console.log(`  ${i + 1}. ${pokemon_1.PokemonManager.getPokemonName(p)} HP:${p.stats.hp}/${p.stats.hp}`);
        }
        console.log('');
        await this.input.waitForKey();
    }
    async openShop(itemIds) {
        if (!this.player)
            return;
        const shopItems = itemIds.map(id => items_json_1.default.find(i => i.id === id)).filter(Boolean);
        while (true) {
            console.log('');
            console.log('═══ 友好商店 ═══');
            ...
            player_1.PlayerManager.removeMoney(this.player, totalCost);
            player_1.PlayerManager.addItem(this.player, selectedItem.id, qty);
    ...
    async showParty() {
        ...
        for (let i = 0; i < this.player.party.length; i++) {
            ...
            const expInfo = pokemon_1.PokemonManager.getExpToNextLevel(p);
            const evoInfo = pokemon_1.PokemonManager.canEvolve(p);
            const evoText = evoInfo.can ? ` [可进化:${evoInfo.requirement}]` : '';
            const natureInfo = pokemon_1.PokemonManager.getNatureInfo(p.nature);
            const natureText = natureInfo.plus ? ` 性格:${natureInfo.name}` : '';
            const abilityText = p.ability ? ` 特性:${p.ability}` : '';
            const ivRating = pokemon_1.PokemonManager.getIVRating(p.ivs);
            ...
            console.log(`   ${ivText}`);
            const chain = pokemon_1.PokemonManager.getEvolutionChain(p.baseId);
            if (chain.length > 1) {
    ...
    async showInventory() {
        ...
        while (true) {
            ...
            const usableItems = this.player.inventory.filter(item => {
                const data = items_json_1.default.find(i => i.id === item.itemId);
                return data && data.type === 'medicine';
            });
            console.log(`[使用道具]`);
            for (let i = 0; i < usableItems.length; i++) {
                const item = usableItems[i];
                const data = items_json_1.default.find(d => d.id === item.itemId);
            ...
            const itemData = items_json_1.default.find(i => i.id === selectedItem.itemId);
            if (!itemData)
                continue;
            console.log(`\n── ${itemData.name} ──`);
            ...
            const targetablePokemon = this.player.party.filter(p => {
                if (itemData.revive)
                    return pokemon_1.PokemonManager.isFainted(p);
                if (itemData.healHp)
                    return !pokemon_1.PokemonManager.isFainted(p) && p.currentHp < p.stats.hp;
                if (itemData.cureStatus)
                    return p.statusEffects.includes(itemData.cureStatus);
                return false;
            });
            if (targetablePokemon.length === 0) {
                ...
                if (itemData.revive)
                    ...
                    const statusNames = { poison: '中毒', burn: '烧伤', paralysis: '麻痹', freeze: '冰冻', sleep: '睡眠' };
            ...
            const targetName = pokemon_1.PokemonManager.getPokemonName(target);
            player_1.PlayerManager.removeItem(this.player, itemData.id, 1);
            if (itemData.revive) {
